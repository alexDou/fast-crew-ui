...
- `pnpm lint` - Check for `oxlint` diagnostics
...
- **OXC** via `oxlint` + `oxfmt` for linting and formatting
...
- These wrap `ky` with base URL from `env.NEXT_PUBLIC_API_URL`
...
`oxfmt` does not currently auto-sort imports, so keep this grouping manually when editing files.
...
- `oxfmt` does not currently reorder Tailwind utility classes, so keep class strings intentional and readable by hand
...
- Auto-runs `oxlint --fix` and `oxfmt` on staged files
...
- OXC lint and format rules fully enforced
- `oxfmt`: `singleQuote: false`, `trailingComma: none`, `printWidth: 100`
